...
{
    // READ VALUES
    int8_t result = _readSensor(dhtPin, DHTLIB_DHT11_WAKEUP, DHTLIB_DHT11_LEADING_ZEROS);
...
{
    // READ VALUES
    int8_t result = _readSensor(dhtPin, DHTLIB_DHT_WAKEUP, DHTLIB_DHT_LEADING_ZEROS);
...
int8_t dht::_readSensor(uint8_t pinIn, uint8_t wakeupDelay, uint8_t leadingZeroBits)
{
    ...
    uint8_t bit = digitalPinToBitMask(pinIn);
    uint8_t port = digitalPinToPort(pinIn);
    volatile uint8_t *PIR = portInputRegister(port);

    // REQUEST SAMPLE
    pinMode(pinIn, OUTPUT);
    digitalWrite(pinIn, LOW); // T-be
    delayMicroseconds(wakeupDelay * 1000UL);
    digitalWrite(pinIn, HIGH); // T-go
    pinMode(pinIn, INPUT);
